...
describe('ColorsDecoder', () => {
  ...
  describe('getNbPositions', () => {
    ...
    describe('same color has correct & wrong position and twice to decode', () => {
      it('should return 1 correct and 1 wrong', () => {
        expect(
          createDecoder(['Red', 'Blue', 'Red']).getNbPositions([
            'Red',
            'Red',
            'Black',
          ])
  ...
  describe('getCorrectPositions', () => {
    ...
    it('when 1 correct & 1 wrong position then return index 0', () => {
      expect(decoder.getCorrectPositions(['Red', 'Green', 'Blue'])).toEqual([
        0,
      ]);
    });

    it('when 2 correct position then return 0 and 2 indexes', () => {
      expect(decoder.getCorrectPositions(['Red', 'Green', 'Yellow'])).toEqual([
        0, 2,
      ]);
    });
  });
  ...
  describe('getNbWrongPositions', () => {
    ...
    describe('none colors match', () => {
      it('should return 0', () => {
        expect(
          decoder.getNbWrongPositions(truePredicate)([
            'Black',
            'Green',
            'Black',
          ])
        ...
        it('should return 0', () => {
          expect(
            createDecoder([
              ...
              'Green',
              'Yellow',
            ]).getNbWrongPositions(excludeFirst)(['Red', 'Red', 'Red', 'Black'])
          ).toBe(0);
        });
      });
    });
    ...
    describe('1 wrong position', () => {
      describe('but twice to decode', () => {
        it('should return 1', () => {
          expect(
            createDecoder(['White', 'Green', 'White']).getNbWrongPositions(
              truePredicate
            )(['Black', 'White', 'Red'])
          ).toBe(1);
        });
        ...
        it('should return 1', () => {
          expect(
            createDecoder([
              ...
              'White',
              'Yellow',
            ]).getNbWrongPositions(truePredicate)([
              'Black',
              'Green',
              'Red',
              'White',
    ...
    describe('2 wrong positions', () => {
      it('should return 2', () => {
        expect(
          decoder.getNbWrongPositions(truePredicate)(['Blue', 'Red', 'Black'])
        ).toBe(2);
      });
      ...
      describe('same color has 2 wrong position but only once to decode', () => {
        it('should return 1', () => {
          expect(
            createDecoder(['Red', 'Blue', 'Red']).getNbWrongPositions(
              truePredicate
            )(['Blue', 'Yellow', 'Blue'])
          ).toBe(1);
        });
    ...
    describe('3 wrong positions', () => {
      ...
      describe('same color has 2 wrong position but only once to decode', () => {
        describe('another color has 1 wrong position', () => {
          it('should return 2', () => {
            expect(
              createDecoder(['Red', 'Blue', 'Yellow']).getNbWrongPositions(
                truePredicate
              )(['Blue', 'Yellow', 'Blue'])
            ).toBe(2);
          });
